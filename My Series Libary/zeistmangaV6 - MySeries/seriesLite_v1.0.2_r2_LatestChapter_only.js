...
/*<![CDATA[*/
const config_SeriesLibrary = {
 labelSeries: "Series",
 maxChapters: 5, // 3 chapter terbaru.. kenapa aku sebut angka 4 sebagai 3 chapter terbaru? ya.. itu karena 1 nya itu series yang dibuang supaya gak keindex.
...
titleZA: Urutkan alfabetis terbalik (Z-A).
  ...
  */
  ...
  typeLabel: ["Manga", "Novel", "Doujin", "WN", "LN", "Raw Novel", "Light Novel (JP)", "Web Novel (CN)", "Web Novel (JP)", "Web Novel (KR)", "Manga", "Manhua", "Manhwa", "Doujinshi", "One Shot", "Webtoon", "One-Shot"], // Type yang ditampilkan
  ...
  image_default: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRLjbMyZwf0T4z9RgOoWe8wdvktWrhgthPIXM2lNIavzniNi-tC90SQMOg",
 ...
 _sessionStorage_seriesFD: false, //pakai session storage
};

// SeriesLibrary: Series dan Chapters
const SeriesLibrary = {
 ...config_SeriesLibrary,
 /*===================BATAS MAINSCRIPT==================*/
 
 fetchFeed(callback, load_series) {
  const script = document.createElement("script");
  const urlMyseries = `/feeds/posts/default/-/${encodeURIComponent(load_series? load_series : this.labelSeries)}?alt=json&callback=${callback}&start-index=${this.feedIndex}&max-results=${this.maxFeedResults}`;
 ...
 async processFeed(feed) {
  ...
  if (this._sessionStorage_seriesFD) {
  ...
  if (feedPost.entry && Array.isArray(feedPost.entry) && feedPost.entry.length > 0) {
   feedPost.entry.forEach((entry) => {
    ...
    let image = (entry.media$thumbnail && entry.media$thumbnail.url) ? entry.media$thumbnail.url.replace('s72', 'w175-h235') :
     (entry.content && entry.content.$t ? this.imageString(entry.content.$t) : this.config.image_default);
 ...
 renderPage() {
  ...
  if (!id_outputDiv) {
   console.warn("Elemen librarySeries tidak ditemukan.");
   return;
  }
  ...
  let htmlOutput = seriesArray
   .map(series => {
    ...
    return `
        <div class="series bookItem flex bg-white dark:bg-gray-800 rounded-lg shadow" hS="${series.image === this.config.image_default ? 'false' : 'true'}">
         <a class="boxT_zero w-28 shrink-0 overflow-hidden" href="${series.url}" title="${series.title}">
         
         <div class="boxS_zero snippet-thumbnail pb-[135%] relative overflow-hidden rounded-lg">   
          <img width="" class="absolute inset-0" loading="lazy" src="${series.image}" alt="${series.title}"/>
          
          ${series.type? `<span class="typeS po_S absolute rounded top-1 end-1 text-xs px-1.5 ${series.type.toLowerCase()}">${series.type}</span>`: ''}
          
          ${series.status? `<span class="statusS po_S absolute rounded bottom-1 start-1 text-xs px-1.5 ${series.status.toLowerCase()}">${series.status}</span>`:''}
	         </div>
	         </a>
	         
          <div class="bChapterS data px-3 py-1.5 grow">
          <div class="text-xs text-accent font-bold">
            ${series.language? `<span class="${series.language.toLowerCase()}">${series.language}</span>`: ''} 
            / ${series.isNew ? '<span class="n">New</span>' : ''}
          </div>
           
           ${series.other? `<span class="n hidden ${series.other.toLowerCase()}">${series.other}</span>`: ''}
           

          <h2 class="titleS line-clamp-1">
 ...
 fetchFeedByLabel(labels, seriesId) {
  ...
  const latestChaptersUrl = `/feeds/posts/default/-/${encodeURIComponent(labels)}?alt=json&callback=window.${latestChapters_callback_name}&max-results=${maxlatestChapters}`;
 ...
 processlatestChapters(feed, seriesId) {
  ...
  let latestChaptersHtml = `
    <ul class="latest-chapters subItem text-sm">
      ${latestChapters
        .map(
          (chapter) => `
        <li class="li_chapter char flex justify-between items-center rounded-lg mb-1 bg-slate-100 text-gray-700 dark:text-gray-400 dark:bg-gray-700 px-1 hover:text-accent duration-150 hover:bg-slate-200 hover:dark:bg-gray-900">
        <div class="chpName">
        <a title="${chapter.title}" href="${chapter.url}" target="_blank">${chapter.title}</a>
        </div>

  		${chapter.isNew ? '<span class="badge new-badge n text-xs text-accent font-bold">New</span>' : ''}

        <time class="text-gray-400 text-xs">${chapter.published}</time>
        </li>
      `
        )
        .join("")}
