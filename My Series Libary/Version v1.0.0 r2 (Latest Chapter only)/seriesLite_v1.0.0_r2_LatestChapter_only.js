      ...
      return `
        <div class="series" hS="${series.image === this.config.image_default ? 'false' : 'true'}">
         ...
         <div class="boxS_zero snippet-thumbnail">   
          ...
          <span class="statusS po_S">${series.status}</span>
          <span class="typeS po_S">${series.type}</span>
  ...
  let latestChaptersHtml = `
    <ul class="latest-chapters">
      ${latestChapters
        .map(
          (chapter) => `
        <li><a href="${chapter.url}" target="_blank">${chapter.title}</a> <time>${chapter.published}</time>${chapter.isNew ? '<span class="badge new-badge">New</span>' : ''}</li>
      `
        )
        .join("")}
    </ul>
...
formatTimestamp(timestamp, format = this.config.modeFormat) {
   ...
   if (match) {filteredTitle = match[0].trim();break;}} this.config.replaceList_ch.forEach(replaceList_ch_array => { filteredTitle = filteredTitle.replace(replaceList_ch_array.target, replaceList_ch_array.change_to);
      });
